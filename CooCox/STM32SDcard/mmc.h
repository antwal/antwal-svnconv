...
#define STATUS_PARAMETER_ERROR 64
   ...
   */
  byte readSectors(MSD_Dev *MSD,byte *buffer, unsigned long sector, byte count);
   ...
   */
  uint16_t sendCommand(MSD_Dev *MSD,const uint8_t command, const uint32_t parameter, const uint8_t  deselect);

	uint32_t sendCommand32(MSD_Dev *MSD,const uint8_t command, const uint32_t parameter, const uint8_t  deselect);
