...
void SS_LOW(const MSD_Dev *MSD){MSD->pio->Out(MSD->cs_pin, 1);}
void SS_HIGH(const MSD_Dev *MSD){MSD->pio->Out(MSD->cs_pin, 0);}
 ...
 * @param[in]  wbuf : pointer to the buffer containing the data to be
 *                    written on the MSD
 * @param[in]  NbrOfBlock : number of block to write
 * @return     The MSD Response:
 *               - COX_ERROR: Sequence failed
 *               - COX_SUCCESS: Sequence succeed
*******************************************************************************/
COX_Status MSD_WriteBuffer(const MSD_Dev *sd, uint32_t adr, const uint8_t* wbuf, uint32_t NbrOfBlock)
{
  uint32_t  Offset = 0;
  COX_Status rvalue = COX_ERROR;
 ...
 *                    from the MSD
 * @param[in]  NbrOfBlock: Number of blocks to read
 * @return     The MSD Response:
 *               - COX_ERROR: Sequence failed
 *               - COX_SUCCESS: Sequence succeed
*******************************************************************************/
COX_Status MSD_ReadBuffer(const MSD_Dev *sd, uint32_t adr, uint8_t* rbuf, uint32_t NbrOfBlock)
{
  /* MSD chip select low */
  SS_LOW(sd);

  /* Send CMD18 (MSD_READ_MULTIPLE_BLOCK) to read multiple block */
  MSD_SendCmd(sd, MSD_READ_MULTIPLE_BLOCK, adr, 0xFF);
  if (MSD_GetResponse(sd, MSD_RESPONSE_NO_ERROR)) {
        return  COX_ERROR;
  }
  do{
  ...
  /* Send CMD12 (MSD_STOP_TRANSMISSION ) to stop transmission */
    MSD_SendCmd(sd, MSD_STOP_TRANSMISSION , 0, 0xFF);
  ...
  sd->spi->ReadWrite(MSD_DUMMY);
  /* Returns the reponse */
  return NbrOfBlock ? COX_ERROR:COX_SUCCESS;
}
...
COX_Status MSD_GetCSDRegister(const MSD_Dev *sd, sMSD_CSD* MSD_csd)
{
  ...
  /* Byte 15 */
  MSD_csd->CRCsd = (CSD_Tab[15] & 0xFE) >> 1;
  MSD_csd->Reserved4 = 1;
...
COX_Status MSD_GetCIDRegister(const MSD_Dev *sd, sMSD_CID* MSD_cid)
{
  ...
  /* Byte 16 */
  MSD_cid->CRCsd = (CID_Tab[15] & 0xFE) >> 1;
  MSD_cid->Reserved2 = 1;
...
void MSD_SendCmd(const MSD_Dev *sd, uint8_t Cmd, uint32_t Arg, uint8_t Crc)
{
  ...
  crc = crc7update(0  , Frame[0]);
   crc = crc7update(crc, Frame[1]);
   crc = crc7update(crc, Frame[2]);
   crc = crc7update(crc, Frame[3]);
   crc = crc7update(crc, Frame[4]);
   crc = (crc << 1) | 1;
...
COX_Status MSD_GetResponse(const MSD_Dev *sd, uint8_t Response)
{
  ...
  while (((tmp = sd->spi->ReadWrite(MSD_DUMMY)) != Response) && Count) {
	  //printf("Re=0x%x\t\t",tmp);
	  if(Response == 0xFE)
	  delay();
    Count--;
...
COX_Status MSD_GoIdleState(const MSD_Dev *sd)
{
  ...
  // Sending cmd1 to check if card is mmc
  if(ACMD1 == 1){
	  counter = 0x1;
	  do{
	  	  MSD_SendCmd(sd, MSD_SEND_OP_COND, 0, 0xFF);
