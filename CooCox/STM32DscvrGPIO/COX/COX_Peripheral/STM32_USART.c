...
void STM_RCC_Configuration(void)
{
    ...
    HSEStartUpStatus = STM_RCC_WaitForHSEStartUp();
    if(HSEStartUpStatus == SUCCESS)
    {
        ...
        tmpreg |= RCC_PLLSource_HSE_Div1 | RCC_PLLMul_9;
        /* Store the new value */
        RCC->CFGR = tmpreg;
        ...
        *(__IO uint32_t *) CR_PLLON_BB = (uint32_t)ENABLE;
        ...
        while(STM_RCC_GetFlagStatus(RCC_FLAG_PLLRDY) == RESET)
        {
        }
        ...
        while(((uint8_t)(RCC->CFGR & CFGR_SWS_Mask)) != 0x08)
        {
        }
    }
}
