...
void init(void)
{
    ...
    lcd_send(0xE4, LCD_CMD);  // Set LCD Vop(Contrast)
    lcd_send(0x45, LCD_CMD);   // New LCD correction factor (Y offset +5)
    ...
    lcd_send(0x09, LCD_CMD);  // ALL black
    _delay_ms(750);
    lcd_send(0x08, LCD_CMD);  // Display blank
    _delay_ms(50);
...
//    for(int i=0;i<LCD_CACHE_SIZE;i++) {
    for(int i=0;i<1020;i++) {					// Display is actually 102 X 80
		lcd_send(0, LCD_DATA);
    }
}
...
//    lcdCacheIdx = (x-1)*6 + (y-1)*84;
      lcdCacheIdx = (x-1)*6 + y*84;		// From Y bank 1
}
