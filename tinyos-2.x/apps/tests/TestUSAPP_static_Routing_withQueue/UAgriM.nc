...
module UAgriM
{
	...
	uses {
		...
		interface Read<uint16_t> as Temperature; // ADC
		interface Read<uint16_t> as LeafWetness;
		interface Read<uint16_t> as Excitation;

		interface Read<uint16_t> as RHSensorTemp;// RHSensor 
		interface Read<uint16_t> as RHSensorHum ;

		interface StaticSend as Radio;           // Static Routing
		interface StdControl as RadioControl;
		interface StaticControl as RouteControl;

		interface Packet as SubPacket;           // AMSenderC
	}	
}
...
implementation
{
	...
	void schedule();			// Function to decrement(1sec) the time left to sense, 
						// for each of the sensing records

	void addRecord(uint8_t, uint32_t); 	// Function to create a Sensor Record

	void sampleRecord(uint8_t);		// Function that calls a read command on the sensor, 
						// whose record index is passed as argument		

	task void sendPacketTask();		// Task function that sends a packet to the Radio
		
	
	//Initiliaze the Sensor Record Structures with default values (Indicating them as Free Records)	
