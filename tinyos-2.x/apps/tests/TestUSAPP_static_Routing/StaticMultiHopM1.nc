...
#define NETWORK_SIZE 10 					// Size of the Mote Network. To be increased if required
...
implementation {
  ...
  /* Parent Table */
  uint16_t  ParentNode[NETWORK_SIZE];				// Array of Parent Nodes. Mote_id being array index, values are the Parent Node Ids.
  ...
  */ 
  ...
  static void init_parentTable() {
    ParentNode[BASE_STATION_ADDRESS] = 0xffff; //TOS_UART_ADDR;
...
//    pMsg->length += 4; 
 ...
 //   pMsg->dest = ParentNode[Index];	
  ...
  static message_t *mForward(message_t *pMsg, uint8_t id) {
    uint8_t transmitbuffer[128], i, len = 0;
   ...
   // pMsg->dest = ParentNode[Index];	
    ...
    if(TOS_NODE_ID == BASE_STATION_ADDRESS) {
      ...
      for (i=0; i<len; i++) 
        transmitbuffer[i] = ((uint8_t*)pMHMsg)[i];
