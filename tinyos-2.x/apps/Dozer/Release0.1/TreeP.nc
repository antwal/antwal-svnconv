...
implementation {
  ...
  event void Boot.booted() {
    ...
    // Initalize the ParentNodes and ChildNodes Tables of Scheduler & Tree Manager 	
...
*/  
  ...
  event message_t* BeaconReceive.receive(message_t* msg, void* payload, uint8_t len) 
  {
    ...
    switch (dmsg->type) {
	case DOZER_BEACON:       // Timestamp potential parents as well as parent for every 1 hour.
	  ...
	  if (TreeConnected && !NbrUpdatePending) {
	    ...
	    if (src == listen_id) { ChildOn = FALSE; }	
	    call TDMASchedule.setParentTimeStamp(src, bmsg, call LocalTime.get());
	  } else {	
	    if ((bmsg->children == MAX_CHILDREN) && (src != gParent_id)) { break; }	
	...
	case DOZER_CONNREQ:   
	  ...
	  if (local.children == MAX_CHILDREN) { break; }
	  child_id = src; 
	  post sendReply();
	  break;
