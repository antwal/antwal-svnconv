...
implementation {
  ...
  command error_t TDMASchedule.getPPB(dozer_parent_t **gParent) 
  {
    ...
    *gParent = ParentNodes[Index]; 
    ...
    // Considering the 32-bit timer counter overflow   	
    if (ParentNodes[Index]->timestamp < call LocalTime.get()) 
      diff = (call LocalTime.get() - ParentNodes[Index]->timestamp);
    else
      diff = ((0x7FFFFFFFL - ParentNodes[Index]->timestamp) + call LocalTime.get() + 1);
    ...
    while(1) {
      ...
      if ((DEFAULT_BEACON_INTERVAL + seed_sum) > diff) { break; }
      rounds++;	
    }
