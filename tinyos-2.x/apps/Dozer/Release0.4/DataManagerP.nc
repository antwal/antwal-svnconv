   ...
   */
    command error_t DataQueueManage.addToQueue(void *in, uint8_t len) {
     dozer_data_header_t *dmsg;
      ...
      //Data Enqueue
      dmsg = (dozer_data_header_t *)call DataSend.getPayload(newmsg, sizeof(dozer_data_header_t));
      dmsg->type = DATA_MSG; 	
      dmsg->more = call DataQueue.size(); 	
      
      memcpy(dmsg->data, in, len);
    ...
    task void sendTask() {
      if (call DataSend.send(dest, msg_ptr, sizeof(dozer_data_header_t)) != SUCCESS) {
        report_problem();
	post sendTask();
      } else {
        sending_packet = TRUE;
    ...
    task void uartSendTask() {
      if (call SerialSend.send(0xffff, uart_msg_ptr, sizeof(dozer_data_header_t)) != SUCCESS) {
        report_problem();
	post uartSendTask();
      } else {
//        sending_packet = TRUE;
	...
	case DATA_MSG:
	  ...
	  if (call DataQueueManage.addToQueue(in->data, len-DOZER_DATA_HEADER_SIZE) == SUCCESS) {
