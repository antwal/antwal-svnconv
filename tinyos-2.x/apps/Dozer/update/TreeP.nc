...
implementation {
  ...
  int8_t PBeaconCount=0, CBeaconCount=0;		// Beacons Sent as a Parent and received as a child 	
  ...
  void startTimer() {
    ...
    if (TreeConnected) {
      ...
      if ((PBeaconCount - CBeaconCount) > 4) {
	call chooseParent();
        return;
      }

      if ((PBeaconCount >= 8) || (CBeaconCount >= 8)) {
	PBeaconCount = PBeaconCount - CBeaconCount;
	CBeaconCount = 0;
      }	
