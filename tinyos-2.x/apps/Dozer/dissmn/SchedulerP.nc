...
implementation {
  ...
  uint8_t gParentTSlot, gChildTSlot, gPIndex = INVALID_VALUE;
  ...
  command error_t TDMASchedule.getPPB(dozer_parent_t **gParent) 
  {
    ...
    if ((gPIndex != INVALID_VALUE) && (ParentNodes[gPIndex]->flag != INVALID_VALUE) &&
	 (ParentNodes[gPIndex]->beacon.hopcount != 0) &&
	 (ParentNodes[gPIndex]->retries == MAX_RETRIES)) {
      ParentNodes[gPIndex]->flag = INVALID_VALUE;
    }
  ...
  async command void TDMASchedule.setParentTDMASlot(uint8_t tslot) 
  {
    ...
    memcpy(&gParentNode, ParentNodes[gPIndex], sizeof(dozer_parent_t)); 	
    startBeaconListenTimer(gPIndex); 
  } 
