...
implementation {
    ...
    Sample.readDone(uint8_t channel,uint8_t channelType,uint16_t data)
    {
        switch (channelType) {
         ...
         case TEMPERATURE:                                       // TEMPERATURE  
		    ...
		    atomic {
		      tmpsnapshot=(OneShotData *)oneshotpacket.data;
		      tmpsnapshot->value[TEMP_TYPE] = data;
		      bitmap &= ~ 	
		    }
		    ...
		    if (bitmap) 
        	    ...
        	    post Send_OneShot_Data();                            // If OneShot Pkt is full call the task to send the data
		 }	
    ...
    */
	...
	case DISSMN_TYPE_ONESHOT:
	    ...
	    atomic {
	      oneshotflag = 1;					 // Set to identify data ready events as OneShot Data events
	    }
