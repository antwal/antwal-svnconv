...
implementation {
  ...
  uint8_t radioOn = FALSE, sendBusy = FALSE;		// Radio is ON and Sending (when both are TRUE)
  bool ParentOn = FALSE, ChildOn = FALSE;		// Specifies the mode of node (Parent or Child or Idle)
  ...
  void startTimer() {
    // Start a Timer to send the Next Beacon Msg
    if (TreeConnected) {
      atomic { 
  ...
  void stop_radio() {   
    ...
    if (!TreeConnected && Listening) { return; }
  ...
  event void TDMASchedule.radioWakeup(uint8_t type, uint16_t node_id) {
	...
	if (i == NBR_TABLE_SIZE) {
	   ...
	   call TDMASchedule.clearTableEntry(listen_id, 1);
	   break;
	}
  ...
  */
  ...
  event void NeighbourUpdateStopTimer.fired() {
    ...
    if ((!TreeConnected) && (NeighbourCount == 0)) {
      SuspendMode = TRUE;
      call NeighbourUpdateTimer.stop();	
    ...
    for (j=0, i=NBR_TABLE_SIZE; (i!=0); i--){
       if ((gParent_id != INVALID_VALUE) && (nbrTable[i-1].parent_id == gParent_id)) {  	
...
*/  
  ...
  task void sendReply() {
    dozer_msg_t *dmsg; 
    dozer_conn_rep_t connRep;				// Reply for a connection Request
  ...
  command error_t chooseParent() {
    ...
    gPIndex = INVALID_VALUE; 	
    local.children = 0;

    if (call BeaconTimer.isRunning()) {
      call BeaconTimer.stop();	
    } else {	
      call BeaconSend.cancel(&sendBuf);
    }	
	...
	case DOZER_BEACON:       
	  ...
	  if (src == listen_id) { 
	    ...
	    if (TreeConnected) {
	      ...
	      if ((src == gParent_id) && (len > sizeof(dozer_beacon_t))) {
