...
implementation {
    ...
    */
	...
	if ((i == LWET_TYPE) || (i == LWET_TYPE1)) {
	  ...
	  // Check the Leafwetness Sensor type
	  if (i == LWET_TYPE1) { ls = 1; lthreshold = LWET_THRESHOLD1; }

          lcount[ls]++;                                 // Increment Leaf Wetness Counter till lcountMax 
    ...
    */
    ...
    event message_t* CmdReceive.receive(message_t* msg, void* payload, uint8_t len)
    {
      ...
      DissmnCtrlMsg_t *pCtrlMsg = (DissmnCtrlMsg_t *)payload;        // Get pointer to payload of Control Msg
      uint16_t i, k;
