...
implementation {
    ...
    uint8_t i2c_data, buff[2];    //the data read from the chip
	...
	if (!call I2CResource.isOwner()) {
	  ...
	  return;
        ...
        if (call I2CPacket.read(0x03, TOS_DIGITAL_I2C_ADDR, 2, buff) == FAIL)
        {
          atomic state=IDLE;
          post read_io();
        }
     ...
     async event void I2CPacket.writeDone(error_t error, uint16_t addr, uint8_t length, uint8_t* data)
     {
	  ...
	  atomic {
            if ( state == SET_OUTPUT_HIGH || state == SET_OUTPUT_LOW || state == SET_OUTPUT_TOGGLE) {
              ...
              //if(post read_io() == FAIL) call Interrupt.enable();
            }
	  }
        //}
	release_i2c();
...
*/
    ...
    {
        call Interrupt.disable();
        if (post read_io() == FAIL) { 
	  call Interrupt.enable();
	  return;
	}
/*						// Commented by sowji
	if (!call I2CResource.isOwner()) {	// Let 'read_io' task take care of acquiring I2C Resource
          call I2CResource.request();
        }
*/
        return;
