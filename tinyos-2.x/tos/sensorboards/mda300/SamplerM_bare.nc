...
#define FLAG_NOT_SET 0
    ...
    void next_schedule(){
        int8_t i;
        int32_t min=SCHEDULER_RESPONSE_TIME;   //minimum time set to 15Sec so that if a new sampling request comes we reply with 15sec delay.
    ...
    void sampleRecord(uint8_t i)
    {
       ...
       if(SampleRecord[i].channelType == TEMPERATURE || SampleRecord[i].channelType == HUMIDITY ) { 
         if(SampleRecord[i].channelType == TEMPERATURE) { call Temp.read(); }               
         if(SampleRecord[i].channelType == HUMIDITY) { call Hum.read(); }               
         return; 
       }
