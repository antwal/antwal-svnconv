...
implementation {
  ...
  enum {
    TIMEOUT_RESET = 11,
    TIMEOUT_14BIT = 250, //210,
    TIMEOUT_12BIT = 250, //55,
    TIMEOUT_8BIT = 250, //11,
  } sht_timeout_t;
  ...
  command error_t SensirionSht15.readStatusReg[ uint8_t client ]() {
    if ( !on ) { return EOFF; }
    if ( busy ) { return EBUSY; } else { busy = TRUE; }
    cmd = CMD_READ_STATUS;
    currentClient = client;
  ...
  /* FIXME: these don't seem to work 
  command error_t SensirionSht15.writeStatusReg[ uint8_t client ]( uint8_t val ) {
    if ( !on ) { return EOFF; }
    if ( busy ) { return EBUSY; } else { busy = TRUE; }
  ...
  error_t performCommand() {
    ...
    case CMD_MEASURE_TEMPERATURE:
      ...
      if ( status & SHT15_STATUS_LOW_RES_BIT ) {
 ...
 */   }
  ...
  event void Timer.fired() {
    ...
    case CMD_MEASURE_TEMPERATURE:
      ...
      busy = FALSE;
      signal SensirionSht15.measureTemperatureDone[currentClient]( FAIL, 0 );
      break;

    case CMD_MEASURE_HUMIDITY:
      // timeout expired with no data interrupt
      busy = FALSE;
      signal SensirionSht15.measureHumidityDone[currentClient]( FAIL, 0 );
      break;
  ...
  task void readSensor() {
    ...
    case CMD_MEASURE_TEMPERATURE:
      busy = FALSE;
      signal SensirionSht15.measureTemperatureDone[currentClient]( SUCCESS, data );
      break;

    case CMD_MEASURE_HUMIDITY:
      busy = FALSE;
      signal SensirionSht15.measureHumidityDone[currentClient]( SUCCESS, data );
      break;
...
/*
  task void signalStatusDone() {
    ...
    case CMD_READ_STATUS:
      busy = FALSE;
      signal SensirionSht15.readStatusRegDone[currentClient]( SUCCESS, status );
      break;
    case CMD_WRITE_STATUS:
      busy = FALSE;
      writeFail = FALSE;
      signal SensirionSht15.writeStatusRegDone[currentClient]( (_writeFail ? FAIL : SUCCESS) );
      break;
    default:
      // shouldn't happen.
      break;
...
//  default event void SensirionSht15.readStatusRegDone[uint8_t client]( error_t result, uint8_t val ) { }
//  default event void SensirionSht15.writeStatusRegDone[uint8_t client]( error_t result ) { }
}
